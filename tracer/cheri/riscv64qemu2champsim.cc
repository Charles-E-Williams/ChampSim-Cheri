...
const std::unordered_map<std::string,uint64_t> TRACE_START_PC {
    ...
    while (std::getline(trace_file,line)) {
        ...
        if (RE2::PartialMatch(line, instr_pattern, &cpu, &pc, &opcode, &mnemonic, &reg_operands)) {
            ...
            try{
                ...
                std::cerr << "Invalid register id: " << reg << std::endl;
            }
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    ProgramTrace trace;
    trace.verbose = (argc >= 4 && std::string(argv[3]) == "-v");
    if (!parse_trace(fileName, oFilename, trace)) {
        std::cerr << "Error: parse_trace failed\n";
        exit(EXIT_FAILURE);
    }
}
