...
uint32_t cheri_ptr_chase::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
...
void cheri_ptr_chase::prefetcher_final_stats()
{
  ...
  fmt::print("  Pointer hit rate:           {:.1f}%\n",
             stat_ptr_found + stat_ptr_not_found > 0
                 ? 100.0 * stat_ptr_found / (stat_ptr_found + stat_ptr_not_found)
                 : 0.0);
  fmt::print("  ---\n");
