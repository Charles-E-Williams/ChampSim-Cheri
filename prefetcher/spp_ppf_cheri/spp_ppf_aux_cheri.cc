...
void spp_ppf_cheri::PPF_Module::do_prefetch(champsim::address addr, champsim::address ip, 
    ...
    uint32_t metadata_in, champsim::capability& cap, double confidence_modifier)
{
	...
	uint8_t num_pf = 0;
	for (unsigned int i=0; i < PAGES_TRACKED; i++) {
		unsigned int j;
		for (j=0; j<i; j++) {
    ...
    //uint64_t cap_top_val    = cheri::capability_top(cap).to<uint64_t>();
    ...
    if(LOOKAHEAD_ON) {
        do {
            ...
            PT.read_pattern(curr_sig, delta_q, confidence_q, perc_sum_q, 
                ...
                addr, base_addr, train_addr, curr_ip, 
                train_delta, last_sig, 
                cache_->get_pq_occupancy().back(), cache_->get_pq_size().back(), cache_->get_mshr_occupancy(), cache_->get_mshr_size(),
                cap_base_val, cap_length_val, cap_offset_val
            );
            ...
            for (uint32_t i = pf_q_head; i < pf_q_tail; i++) {
                ...
                if (champsim::page_number{addr} == champsim::page_number{pf_addr}) { // Prefetch request is in the same physical page
                    ...
                    if ( num_pf < ceil(((cache_->get_pq_size().back())/distinct_pages)) ) {					
                        if (FILTER.check(pf_addr, train_addr, curr_ip, 
                            fill_level, train_delta + delta_q[i], 
                            last_sig, curr_sig, confidence_q[i], perc_sum, (depth-1),
                            cap_base_val, cap_length_val, cap_offset_val)) 
                        {
...
void spp_ppf_cheri::PPF_Module::SIGNATURE_TABLE::read_and_update_sig(champsim::page_number page, offset_type page_offset, uint32_t &last_sig, uint32_t &curr_sig, typename offset_type::difference_type &delta)
{
    uint32_t set = static_cast<uint32_t>(spp_ppf_cheri::get_hash(page.to<uint64_t>()) % ST_SET), match = ST_WAY;
        ...
        for (uint32_t way = 0; way < PT_WAY; way++) {
            ...
            if (pf_conf && pf_q_tail < parent_->cache_->get_mshr_size() && (perc_sum < PERC_THRESHOLD_HI) ) {
                ...
                if (champsim::page_number{champsim::address{addr}} == champsim::page_number{pf_addr}) { // Prefetch request is in the same physical page
                	parent_->FILTER.check(pf_addr, train_addr, curr_ip, SPP_PERC_REJECT, train_delta + delta[set][way], last_sig, 
...
bool spp_ppf_cheri::PPF_Module::PREFETCH_FILTER::check(champsim::address check_addr,
    ...
    switch (filter_request) {
		...
		case SPP_PERC_REJECT: // To see what would have been the prediction given perceptron has rejected the PF
            if ((valid[quotient] || useful[quotient]) && remainder_tag[quotient] == remainder) { 
				...
				if (train_neg) {
                    ...
                    cap_base_reject[quotient_reject] = cap_base_val;
                    cap_length_reject[quotient_reject] = cap_length_val;
                    cap_offset_reject[quotient_reject] = cap_offset_val;
				}
                ...
                if(SPP_DEBUG_PRINT) {
					...
					std::cout << " More Recorded Metadata: Addr: " << std::hex << address_reject[quotient_reject] << std::dec << " PC: " << pc_reject[quotient_reject] 
                    << " Delta: " << delta_reject[quotient_reject] << " Last Signature: " 
                    << last_signature_reject[quotient_reject] << " Current Signature: " 
                    << cur_signature_reject[quotient_reject] << " Confidence: " << confidence_reject[quotient_reject] 
                    << std::hex << "Cap Base: 0x" << cap_base_val << " Cap Length: 0x" << cap_length_val << " Cap Offset: 0x" << cap_offset_val  << std::dec
                    << std::endl;
                }
			}
			break;
		...
		case SPP_L2C_PREFETCH:
            if ((valid[quotient] || useful[quotient]) && remainder_tag[quotient] == remainder) { 
                ...
                cap_base_pf[quotient] = cap_base_val;
                cap_length_pf[quotient] = cap_length_val;
                cap_offset_pf[quotient] = cap_offset_val;
                ...
                if(SPP_DEBUG_PRINT) {
					...
					std::cout << " More Recorded Metadata: Addr:" << std::hex << address[quotient] << std::dec << " PC: " << pc[quotient] << " Delta: " 
                    << delta[quotient] << " Last Signature: " << last_signature[quotient] 
                    << " Current Signature: " << cur_signature[quotient] << " Confidence: " << confidence[quotient] << 
                    std::hex << "Cap Base: 0x" << cap_base_val << " Cap Length: 0x" << cap_length_val << " Cap Offset: 0x" << cap_offset_val   << std::dec
                    << std::endl;
                }
            }
            ...
            if ((remainder_tag[quotient] == remainder) && (useful[quotient] == 0)) {
                ...
                if (valid[quotient]) {
					...
					parent_->PERC.perc_update(address[quotient], pc[quotient],
                        ...
                        cap_base_pf[quotient], cap_length_pf[quotient], cap_offset_pf[quotient]
                    );
				...
				if (valid_reject[quotient_reject] && remainder_tag_reject[quotient_reject] == remainder_reject) {
					...
					if (train_neg) {
						...
						parent_->PERC.perc_update(address_reject[quotient_reject], pc_reject[quotient_reject],
                                ...
                                delta_reject[quotient_reject], last_signature_reject[quotient_reject], cur_signature_reject[quotient_reject], 
                                confidence_reject[quotient_reject], la_depth_reject[quotient_reject], 0, perc_sum_reject[quotient_reject],
                                cap_base_reject[quotient_reject], cap_length_reject[quotient_reject], cap_offset_reject[quotient_reject]
                                );
        ...
        case L2C_EVICT:
            ...
            if (valid[quotient] && !useful[quotient]) {
				...
				parent_->PERC.perc_update(address[quotient], pc[quotient], 
                    pc_1[quotient], pc_2[quotient], pc_3[quotient], delta[quotient], 
                    last_signature[quotient], cur_signature[quotient], confidence[quotient], 
...
void spp_ppf_cheri::PPF_Module::GLOBAL_REGISTER::update_entry(uint32_t pf_sig, uint32_t pf_confidence, offset_type pf_offset, typename offset_type::difference_type pf_delta) 
{
...
void spp_ppf_cheri::PPF_Module::get_perc_index(champsim::address base_addr,
...
int32_t	spp_ppf_cheri::PPF_Module::PERCEPTRON::perc_predict(champsim::address base_addr, champsim::address ip, 
    ...
    typename offset_type::difference_type cur_delta, uint32_t last_sig, 
    uint32_t curr_sig, uint32_t confidence, uint32_t depth, 
    uint64_t cap_base_val, uint64_t cap_length_val, uint64_t cap_offset_val)
{

	uint64_t perc_set[PERC_FEATURES];
	// Get the indexes in perc_set[]
	parent_->get_perc_index(base_addr, ip, ip_1, ip_2, ip_3, 
                            cur_delta, last_sig, curr_sig, confidence, depth,
                            cap_base_val, cap_length_val, cap_offset_val, perc_set);
	
	int32_t sum = 0;
	for (unsigned int i = 0; i < PERC_FEATURES; i++) {
...
void spp_ppf_cheri::PPF_Module::PERCEPTRON::perc_update(champsim::address base_addr, champsim::address ip, 
    champsim::address ip_1, champsim::address ip_2, champsim::address ip_3, 
    typename offset_type::difference_type cur_delta, uint32_t last_sig, uint32_t curr_sig, uint32_t confidence, 
    ...
    uint64_t cap_base_val, uint64_t cap_length_val, uint64_t cap_offset_val)
{
	...
	if (direction && sum > static_cast<int32_t>(NEG_UPDT_THRESHOLD) && sum < static_cast<int32_t>(POS_UPDT_THRESHOLD)) {
