...
void ampm_cheri::add_to_map(champsim::address v_addr, const champsim::capability& cap, bool prefetch)
{
  ...
  if (existing.has_value()) {
    ...
    existing->cap_top  = cap.base.to<uint64_t>() + cap.length.to<uint64_t>();
    ...
    for (int i = 1, pf_count = 0; i <= static_cast<int>(lines_per_zone()) && pf_count < degree; i++) {
      ...
      if (check_map(champsim::address{neg_step}, cap, false) &&
        ...
        if (va_block != champsim::block_number{pos_step}) {
          ...
          if (same_page) {
            uint64_t pa_candidate = (pa_val & ~page_mask) | (va_cand_val & page_mask);
            champsim::address pf_addr{pa_candidate};
